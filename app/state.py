...
class TrainState:
    ...
    def __init__(
        ...
        # キューの各アイテムは (msg, replace_last) のタプル。
        # replace_last=True の場合、ドレイン側がウィジェットの最終行を置換する
        # （tqdm "steps:" 行の重複表示抑制用。runner.py 参照）。
        ...
        # 学習終了時刻（完了・エラー・停止いずれかで launch_training() が設定する）
        # None の間は実行中とみなし、モニターの経過時間表示を更新し続ける。
        # 値が設定された後は経過時間の計算をこの時刻で固定する。
        self.training_end_time: datetime.datetime | None = None
