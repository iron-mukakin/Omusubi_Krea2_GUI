...
class MonitorGraph:
    ...
    def _update_time_panel(self, s: "TrainState") -> None:
        ...
        training_end_time が設定されている（学習が完了・エラー・停止のいずれかで
        終了した）場合は、その時刻を基準に経過時間を固定する。
        実行中は datetime.now() を都度使用してリアルタイムに更新する。
        """
        start = getattr(s, "training_start_time", None)
        if start is None:
            return

        end = getattr(s, "training_end_time", None)
        ...
        if end is not None:
            # 終了済み: 残り時間・完了予測は「終了」固定表示にする
            self._time_labels["eta_remain"].configure(text="終了")
            self._time_labels["eta_clock"].configure(text=end.strftime("%H:%M:%S"))
            return
