...
from .state import TrainState, LAYER_GROUP_DEFS
...
def build_network_tab(parent: ttk.Frame, s: TrainState) -> None:
    ...
    rb_text.pack(side=tk.LEFT, padx=(4, 0))
    rb_group = ttk.Radiobutton(
        submode_frame, text="層グループ選択",
        variable=s.layer_input_mode, value="group",
    )
    rb_group.pack(side=tk.LEFT, padx=(8, 0))
    ...
    # ── ラジオボタンのコマンドをバインド（排他制御）──────────────
    rb_text.configure(command=lambda: _on_submode_change(s, args_entry, text_frame, group_frame))
    rb_group.configure(command=lambda: _on_submode_change(s, args_entry, text_frame, group_frame))
...
def _build_group_selector(
    ...
    for i, (key, label, _pat) in enumerate(LAYER_GROUP_DEFS):
        ...
        ttk.Checkbutton(
            ...
            command=lambda: _sync_group_preview(s, args_entry),
...
def _set_all_groups(s: TrainState, value: bool, args_entry: ttk.Entry) -> None:
    for var in s.layer_groups.values():
        var.set(value)
    _sync_group_preview(s, args_entry)
...
def _sync_group_preview(s: TrainState, args_entry: ttk.Entry) -> None:
    """グループ選択 → network_args StringVar へ同期（読み取り専用表示）。"""
...
def _on_submode_change(
    s: TrainState,
    args_entry: ttk.Entry,
    text_frame: ttk.Frame,
    group_frame: ttk.Frame,
) -> None:
    """サブモード切替時に args_entry の編集可否を排他切替する。

    text モード: Entry を通常編集可能にする。
    group モード: Entry を readonly にしてグループ状態を反映する。
    """
...
def _on_target_change(
    ...
    """Combobox 変更時にヒント・カスタムフレームを更新する。"""
    ...
    if target == "all":
        ...
        _on_submode_change(s, args_entry, text_frame, group_frame)
